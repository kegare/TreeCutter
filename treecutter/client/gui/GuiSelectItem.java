...
public class GuiSelectItem extends GuiScreen
{
	...
	public void drawScreen(int mouseX, int mouseY, float ticks)
	{
		...
		if (single)
		{
			name = I18n.format("treecutter.gui.select.item");
		}
		else
		{
			name = I18n.format("treecutter.gui.select.item.multiple");
		}
		...
		if (!single && !itemList.selected.isEmpty())
		{
			...
			{
				drawString(fontRenderer, I18n.format("treecutter.gui.select.item.selected", itemList.selected.size()), 5, 5, 0xEFEFEF);
			}
	...
	protected class ItemList extends GuiListSlot
	{
		...
		{
			return TreeCutterUtils.itemFilter(itemMeta, filter);
		}
	}
}