...
import treecutter.client.config.TreeCutterGuiFactory;
import treecutter.util.ArrayListExtended;
...
public class GuiSelectBlock extends GuiScreen
{
	...
	public void initGui()
	{
		...
		if (detailInfo == null)
		{
			detailInfo = new GuiCheckBox(1, 0, 5, I18n.format("treecutter.gui.detail"), true);
		}

		detailInfo.setIsChecked(TreeCutterGuiFactory.detailInfo);
		detailInfo.x = width / 2 + 95;

		if (instantFilter == null)
		{
			instantFilter = new GuiCheckBox(2, 0, detailInfo.y + detailInfo.height + 2, I18n.format("treecutter.gui.instant"), true);
		}

		instantFilter.setIsChecked(TreeCutterGuiFactory.instantFilter);
		...
		if (button.enabled)
		{
			switch (button.id)
			{
				case 0:
					...
					break;
				case 1:
					TreeCutterGuiFactory.detailInfo = detailInfo.isChecked();
					break;
				case 2:
					TreeCutterGuiFactory.instantFilter = instantFilter.isChecked();
					break;
				default:
	...
	public void drawScreen(int mouseX, int mouseY, float ticks)
	{
		...
		if (single)
		{
			name = I18n.format("treecutter.gui.select.block");
		}
		else
		{
			name = I18n.format("treecutter.gui.select.block.multiple");
		}
		...
		{
			drawHoveringText(fontRenderer.listFormattedStringToWidth(I18n.format("treecutter.gui.detail.hover"), 300), mouseX, mouseY);
		}
		else if (instantHoverChecker.checkHover(mouseX, mouseY))
		{
			drawHoveringText(fontRenderer.listFormattedStringToWidth(I18n.format("treecutter.gui.instant.hover"), 300), mouseX, mouseY);
		}
		...
		if (!single && !blockList.selected.isEmpty())
		{
			...
			{
				drawString(fontRenderer, I18n.format("treecutter.gui.select.block.selected", blockList.selected.size()), 5, 5, 0xEFEFEF);
			}
	...
	protected class BlockList extends GuiListSlot
	{
		...
		{
			return TreeCutterUtils.blockFilter(blockMeta, filter);
		}
	}
}